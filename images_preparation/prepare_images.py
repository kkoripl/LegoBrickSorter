...
def prepare_images():
    images_dir = os.path.abspath(AppParams.cropped_img_dir)
    prep_img_dir = os.path.abspath(AppParams.prep_img_dir)
...
def resize_and_save_images(images_paths, actual_dir, target_dir):
    for img_path in images_paths:
        ...
        layer = layer.resize(AppParams.img_size, resample=Image.ANTIALIAS)
        layer.save(os.path.join(target_dir, img_path))


def get_brick_dirs():
    return [subdir for subdir in os.listdir(os.path.abspath(AppParams.cropped_img_dir))]
